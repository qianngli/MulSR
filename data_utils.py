...
import os
from imresize import downscale, anisotropic_gaussian_kernel, isotropic_gaussian_kernel, fix_gaussian_noise
...
def train_lr_transform_0_5(interpolation):
    return compose([ 
        Resize_train(2, interpolation)
    ])
...
class TrainsetFromFolder(Dataset):
    def __init__(self, dataset, dataset_dir,  upscale_factor, interpolation = 'Bicubic', patchSize = 28, crop_num = 48):
        ...
        self.lr_transform_0_5 = train_lr_transform_0_5(interpolation)
...
class ValsetFromFolder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #hr_rgb = torch.from_numpy(hr_rgb)[:,:W,:H]
       ...
       # W, H = mrop(self.scale, hsi.shape[1], hsi.shape[2]) 
        ...
        #hsi = torch.from_numpy(hsi)[:,:W,:H]
        #hr_rgb = torch.from_numpy(rgb)[:,:W,:H]
